...
export default function EvilEye({
  ...
  useEffect(() => {
    ...
    function onMouseMove(e: MouseEvent) {
      ...
      mouse.tx = ((e.clientX - rect.left) / rect.width) * 1.25 - 1;
      mouse.ty = -(((e.clientY - rect.top) / rect.height) * 1.25 - 1);
    }
    ...
    // --- BLINK LOGIC SETUP ---
    // Start the pupil at 0 (closed)
    let currentPupilSize = 0.0;
    let targetPupilSize = 0.0;
    let isRunning = true;
    ...
    program = new Program(gl, {
      ...
      uniforms: {
        ...
        uPupilSize: { value: currentPupilSize }, // Initialized closed
    ...
    // --- BLINK SEQUENCE CONTROLLER ---
    const runBlinkSequence = async () => {
      // Pick random amount of blinks (2 to 4)
      const numBlinks = Math.floor(Math.random() * 3) + 2;

      for (let i = 0; i < numBlinks; i++) {
        // 1. Target opens to the full prop size
        targetPupilSize = pupilSize;
        
        // 2. Wait a random amount of time (0.5s to 1.5s)
        const waitTime = Math.random() * 1000 + 500;
        await new Promise(res => setTimeout(res, waitTime));
        if (!isRunning) return;

        // 3. Target slams shut (Blink)
        targetPupilSize = 0.0;
        
        // 4. Hold closed for a quick 150ms
        await new Promise(res => setTimeout(res, 150));
        if (!isRunning) return;
      }

      // 5. Sequence complete, lock the eye open
      targetPupilSize = pupilSize;
    };
    ...
    function update(time: number) {
      ...
      // Smoothly open and close the pupil
      // The 0.15 multiplier controls how fast the eyelid moves
      currentPupilSize += (targetPupilSize - currentPupilSize) * 0.15;
      
      // Update Uniforms
      program.uniforms.uMouse.value = [mouse.x, mouse.y];
      program.uniforms.uTime.value = time * 0.001;
      program.uniforms.uPupilSize.value = currentPupilSize; // Send new size to Shader
    ...
    return () => {
      isRunning = false; // Stop the async blink loop if user navigates away
